...
In the context of regression, a natural goal is to estimate a regression function $ \theta $ such that the $ L^2 $-risk or mean squared error $ E(Y - \theta(X))^2 $ for  n observation $ O = (Y, X) $ is minimized. It turns out that the conditional mean $ x \mapsto E(Y \mid X = x) $ minimizes the squared error, but the conditional mean can only be identified if we know the underlying data-generating process $ P \in \mathcal{P} $ for which $ O \sim P $. We typically make certain assumptions about the statistical model, $\mathcal{P}$, in which we believe $P$ resides. For instance, we might assume that $\mathcal{P}$ is a curved exponential family. In doing so we are able to identify through maximum likelihood techniques, the parameters of the distribution $ P $ and achieve an asymptotic convergence rate of $ O(1/n) $. \cite{lauritzen2022statistics}[ch. 5]
...
However, note that formally $ \btheta(D_n) $ is a stochastic variable since $ D_n $ is stochastic. A learner is, therefore, a map from a background space, $ \Omega $, to the parameter space, $ \Theta $. In practice, we would have observed $ O_3(\omega) = o_1, \dots, O_n(\omega) = o_n $ for a specific $ \omega $, and subsequently, we can apply our learning algorithm on $ D_n(\omega) = (o_1 , \dots , o_n) $, which is a particular instance of a dataset. We will refer the quantity, $ \btheta(D_n(\omega)) $, as a \textbf{fitted learner}. 
...
\end{align*}
We can, therefore, write $ \btheta(P_n)$ as an alternative representation of the learner $\btheta(D_n)$, by adjusting the notation slightly without introducing ambiguity. The motivation for using this notation will become clearer in the subsequent section, where we introduce the discrete super learner.
...
Given a split variable $S$ for our data $D_n$, the risk of our learner on the cross-validation test data can be expressed as
 ...
 \begin{definition}[Expected loss averaged over splits \cite{vaart06}]
     Given the data $ D_n $ and some split-variable $ S $, the averaged expected loss of some learner, $ \theta_{q}(P_{n,s}^{0}) $, in a library, $ \lib_{\ml}(P_n) $, applied to our training data $ P_{n, S}^{0} $ is 
    \begin{align*}
...
\begin{align*}
    E_S R( \btheta_\q(P_{n,S}^{0}), P) &= \sum_{s \in \operatorname{supp}(S)} R(\theta_{q}(P_{n, s}^{0}), P) \cdot P(S = s) 
\end{align*}

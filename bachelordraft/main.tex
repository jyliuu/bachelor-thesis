...
\end{align*}
Where we use Fubini in the last equality. Recall that $ \Sn \indep D_n $, so we can consider $ n_1 $ as fixed given $ D_n $, now applying \Cref{finitesamplebound} to the expression above with $ n = n_1 $ yields 
\begin{align*}
&E_\Sn\frac{1 + \delta}{\sqrt{n_1} } E_{D_n} \max_{\la \in \lib} \int L(O, \la(P_{n, \Sn}^{0})) d (G_{n,\Sn}^{1} - \frac{\delta }{1 + \delta} \sqrt{n_1} P)(O) \\
...
\begin{proof}
    We let $ \delta $ depend on $ n $, define $ \delta_n := 1/\log(n) $.  By choosing $ p = 1 $ in \Cref{finitesample} and substituting $ \delta $ with $ \delta_n $, we obtain
    \begin{align*}
...
\begin{example}[Regression]
   ...
   Since the integrand is positive, the integral is zero if and only if $ Y = E(Y \mid X) $ almost surely. That is only the case when $ Y $ is deterministic given $ X $, i.e. $ Y $ is $ X $-measurable. Assuming, therefore, that $ Y $ is not deterministic given $ X $, then we note that by the fact that the quadratic loss is strictly proper due to \Cref{minrisk}, it holds for any $ \theta \in \Theta $ that
   \begin{align*}
...
\end{figure}
\Cref{fig:esldiagram} from \citet{van2007super} illustrates how the ensemble super learner is fitted in practice. Step 3 in the diagram is the fitting of the meta learning algorithm, $ \Meta $, to the level 1 data. The result is a meta learner $ m $, which we have denoted in our setup as $ \phi $.  

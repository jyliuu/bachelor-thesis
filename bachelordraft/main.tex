...
\begin{example}[Cross-validation loss]
   The test size $ n_1 $ is formally a random variable as it depends on $ S $
   Hold one out cross validation
   \begin{align*}
       E_S R(\theta(P_{n, S}^{0} , P_{n, S}^{1}))= E_S \frac{1}{n_1} \sum_{i : S_i = 1} L(o_i, \theta) = E_S L(o_{S_t}, \theta) = \frac{1}{n} \sum_{i = 1}^{n} L(o_i, \theta) 
   \end{align*}

...
We will consider the parameter set $ \Theta = \{\theta \mid \theta : \mathcal{X} \to [0,1]\} $, which represents the set of regression functions that map from our covariates to the probability interval. We define the quadratic loss and the corresponding risk that we wish to minimize 
...
\end{align*}
\end{definition}
Our natural aim would be to find the optimal parameter value $\theta^* \in \Theta$ that minimizes the $L^2$-risk $R: \Theta \to \mathbb{R}$ given as 
\begin{align*}
...
In the following section we introduce the terminology ``learning algorithm'' which corresponds to the map from our data to our parameter space. 
...
\end{definition}
We hereby refer to the result of applying the learner algorithm to our data as the fitted learner.

We slightly abuse the notation by denoting the learning algorithm as $ \theta $. Let $ D_n = (O_1 , \dots , O_n) $ be our data, then the fitted learner $\theta(D_n) $ is formally represented as a stochastic variable since $ D_n $ is stochastic, it is formally a map from the background space. 

But by observing A fitted learner The abuse of notation is similar to 

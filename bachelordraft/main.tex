...
\begin{align*}
    R(\btheta) = \int L(o, \btheta)  dP(o) = EL(O_1, \btheta),
\end{align*}
...
\begin{align*}
    D_n \mapsto \hat{\btheta}(D_n) \in \btheta,   
\end{align*}
...
\begin{align*}
    ...
    P_{n,S}^{1} &= \frac{1}{1-n_0} \sum_{i: S_i = 1} \delta_{O_i} 
\end{align*}
...
\begin{definition}[Oracle selector]
    The oracle selector is a function $ \tilde{\q}: \mathcal{O}^{n} \to \{1,\ldots,k\} $  which finds the learner that minimizes the true risk given our data $ O_1 , \ldots , O_n \in O$. 
    \begin{align*}
        \tilde{\q}(O_1 , \ldots, O_n) = \argmin_{1 \leq \q \leq k} E_S R( \btheta _\q (P_{n,S}^0 )) 
    \end{align*}
...
\begin{definition}[Cross-validation risk of $ \q $'th learner averaged over splits]
    Given the data $ O_1, \ldots, O_n \in \mathcal{O} $ and a set of learners $ \{ \btheta_{\q}(P_{n, S}^{n}) \mid 1 \leq \q \leq k \}, k \in \mathbb{N} $. The cross-validation risks of these learners averaged over some split-variable $ S $ is given as a function of $ \q $ 
    \begin{align*}
        \q \mapsto E_S \int L(o, \btheta_{\q}(P_{n,S}^{0}) ) dP_{n, S}^{1}(o) = E_S \hat{R}( \btheta_\q(P_{n,S}^{0})) 
    \end{align*}
...
\begin{definition}[Cross-validation selector]
    The cross-validation selector is a function $ \hat{\q}: \mathcal{O}^{n} \to \{1,\ldots,k\} $  which finds the learner that minimizes the cross-validation risk given our data $ O_1 , \dots , O_n \in \mathcal{O} $. 
    \begin{align*}
        \hat{\q}(O_1 , \dots , O_n) = \argmin_{1 \leq \q \leq k} E_S \hat{R} ( \btheta _\q (P_{n,S}^0 )) 
    \end{align*}

...
\begin{align*}
    P_n(A) = \frac{1}{n} \sum_{i = 1}^{n} 1_{A}(O_i)\qquad \text{ for } A \in \mathcal{A}.
\end{align*}
...
\end{example}
We would now like to consider the scenario where we have a library (set) of estimator algorithms, $ \btheta_1 , \ldots, \btheta_n $. From these algorithms, we can define the set of estimators $ \{ \hat{\theta} _{\cl} = \btheta_{\cl}(P_n) | 1 \leq \cl \leq p \} $, where our goal is to find $ \hat{\theta}_{ \hat{\cl} }(P_n) $, which denotes the estimator that minimizes $ R $ and $ \hat{\cl}  $ may depend on the observations. 
...
\begin{definition}[Oracle selector]
    The oracle selector is a function $ \tilde{\cl}: \mathcal{X}^{n} \to \{1,\ldots,p\} $  which finds the estimator that minimizes the true risk given our data $ X \in \mathcal{X}^{n}$. 
    \begin{align*}
...
\begin{definition}[Cross-validation selector]
    The cross-validation selector is a function $ \hat{\cl}: \mathcal{X}^{n} \to \{1,\ldots,p\} $  which finds the estimator that minimizes the cross-validation risk given our data $ X \in \mathcal{X}^{n} $. 
    \begin{align*}

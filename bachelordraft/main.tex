...
\begin{proof}
    ...
    \end{align*}
    As the inequality holds for all $ \delta > 0 $, we can just set it to 0 (formally one would perhaps choose $ \delta_n $ decreasing in $ n $). The problem now lies in determining how fast the oracle risk converges, see that by setting $ \delta = 0 $ the inequality becomes 
    \begin{align*}
...
\begin{example}[Binary regression]
   In binary regression with quadratic loss, the risk for a consistent learner of the true regression $ x \mapsto P(Y = 1 \mid X = x) $ can never be zero unless the binary outcome is deterministic given $ x $. We have 
   \begin{align*}
       R(\theta(P_n) , P) = \int (Y - \theta(P_n)(X))^2 dP \overset{P}{\longrightarrow}   \int (Y - E(Y \mid X))^2 d P 
   \end{align*}
   How to prove above?
\end{example}

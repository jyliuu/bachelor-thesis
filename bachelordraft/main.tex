...
\setlength{\headheight}{13.6pt} % Set the head height to a larger value
\addtolength{\topmargin}{-1.6pt} % Adjust the top margin to compensate for the increased head height
\fancyhf{} % Clear header and footer
\fancyhead[L,RO]{\thepage} % Page number on the right side of even pages, left side of odd pages
\fancyhead[LO]{\leftmark} % Chapter title on the left side of odd pages
...
\newenvironment{codeavailability}{%
  \renewcommand{\abstractname}{Code availability}
  \begin{abstract}
}{%
  \end{abstract}
}
...
\begin{document}
\begingroup
    \fontencoding{T1}\fontfamily{lmr}\selectfont % Set the font
    \maketitle
    \thispagestyle{empty} 
    \subfile{to}
    \newpage
...
\end{equation}
\Cref{minrisk} shows that the minimum risk is achieved by the conditional probability $ x \mapsto P(Y = 1\mid X = x) $; we also say that the quadratic loss is \textit{strictly proper} \parencite{gneiting2007strictly}. 
...
of size $ \ml $. Our natural goal is to find $ \tilde{\la} \in \lib $ such that $ \tilde{\la}(P_n) $ achieves the lowest possible risk 
...
\begin{definition}[Discrete super learner]
    ...
    \end{align*}
\end{definition}
Formally, the map above is a random map as $ P_n $ is stochastic. The discrete super learner is not a specific learner among the learners in the library, but the resulting algorithm from applying the cross-validation selector, fitted to the dataset. The pseudocode for the discrete super learner fitting procedure can be found in \Cref{sim_dsl}.
...
\begin{proof}
    See appendix.
\end{proof}
\noindent To provide the bounds for the risk we will introduce Bernstein pairs. They are used in the Bernstein inequality to control for the variables $ G_nf $ for $ f \in \mathcal{F} $, which is a critical ingredient in \Cref{finitesamplebound}. The exact details in the proof of \Cref{finitesamplebound} requires an introductory course to empirical processes, which is omitted for the purposes of this thesis.  
...
\begin{example}[Binary regression] \label{ex:bernsteinexample}
    Consider binary regression with quadratic loss. Let $ \btheta \in \Theta $ be arbitrary, then a Bernstein pair for the function $ f_{\btheta}(O) = L(O, \btheta) = (Y - \theta(X))^2$ can be found by applying \Cref{unifbernstein}. By requiring that $ \theta(x) \in [0,1] $, then it is clear that $ f $ is bounded between $ 0 $ and $ 1 $ for all $ O \in \mathcal{O} $ since $ Y \in \{0,1\} $. We also note that the Bernstein pair for $ f_{\btheta} $ satisfies $ 0 \leq \norm{f_\theta}_{\infty} \leq 1 $ and $ 0 \leq \frac{3}{2}Pf_{\btheta}^2 \leq 1 $, so they are bounded. 
...
\begin{proof}[Proof]
    We will apply \Cref{finitesamplebound} to the second and third terms on the left hand side of the inequality in \Cref{finitesampledecomp}. Let $ \mathcal{F} = \{O \mapsto L(O, \la(P_{n,\Sn}^{0})) \mid \la \in \lib\}$, be the collection of functions obtained by applying the loss $ L $ to the algorithms in our library $ \lib$ fitted on the training data. Note that $ \mathcal{F} \subseteq \{O \mapsto L(O, \btheta) \mid \btheta \in \Theta\} $, and since $ R(\btheta, P) \geq 0 $ for every $ \btheta \in \Theta $ it follows that $ Pf \geq 0 $ for every $ f \in \mathcal{F} $.

First, we take the expectation with respect to $ D_n $ on both sides in \Cref{finitesampledecomp}. For the second term we have 
\begin{align*}
...
\end{align*}
Where we use Fubini in the last equality. Recall that $ \Sn \indep D_n $, so we can consider $ n_1 $ as fixed given $ D_n $. By applying \Cref{finitesamplebound} to the expression above with $ n = n_1 $ yields 
\begin{align*}
&E_\Sn\frac{1 + \delta}{\sqrt{n_1} } E_{D_n} \max_{\la \in \lib} \int L(O, \la(P_{n, \Sn}^{0})) d (G_{n,\Sn}^{1} - \frac{\delta }{1 + \delta} \sqrt{n_1} P)(O) \\
...
\end{align*}
Where for the third inequality we take the $ \sup $ over $ \Theta $. We can also bound the third term with the same expression above as \Cref{finitesamplebound} is also valid for $ -L $. It is now immediate from \Cref{finitesampledecomp} that 
\begin{align*}
...
In the following corollary we assume that the Bernstein pairs $ (M(\btheta), v(\btheta)) $ are bounded over $ \Theta $, thus taking the supremum over $ \Theta $ will not result in the remainder being infinite. The assumption is indeed valid for binary regression as we have remarked upon in \Cref{ex:bernsteinexample}. 
...
\begin{proof}
    We let $ \delta_n := 1/\log(n) $.  By choosing $ p = 1 $ in \Cref{finitesample} and substituting $ \delta $ with $ \delta_n $, we obtain
    \begin{align*}
...
\begin{example}[Regression]
   ...
   Since the integrand is positive, the integral is zero if and only if $ Y = E(Y \mid X) $ almost surely. This is only the case when $ Y $ is deterministic given $ X $, i.e. $ Y $ is $ X $-measurable. Assuming, therefore, that $ Y $ is not deterministic given $ X $, then we note that by the fact that the quadratic loss is strictly proper due to \Cref{minrisk}, it holds for any $ \theta \in \Theta $ that
   \begin{align*}
...
\end{figure}
\Cref{fig:esldiagram} from \citet{van2007super} illustrates how the ensemble super learner is fitted in practice. Step 3 is where the meta learning algorithm, $ \Meta $, applied to the level 1 data. The result is a meta learner $ m $, which we have denoted in our setup as $ \phi $. The pseudocode for the ensemble super learner fitting procedure can be found in \Cref{sim_esl}.
...
\end{align}
\Cref{eq:meta_learning_algorithm_cv} shows that the cross-validation selector, $ \hat{a}_n $, is exactly the objective of the meta learning algorithm defined in \eqref{eq:meta_learning_algorithm}. 
...
The following finite sample oracle inequality is a direct consequence of the oracle inequality for the discrete super learner, \Cref{finitesample}.
\begin{theorem}[Finite sample oracle inequality]
    Let $ \lib $ be a library of learning algorithms of size $ k $. Let $ \esl_{ \hat{a}_{n} } $ be the ensemble super learner over $ \lib $ obtained by applying the cross-validation selector over a finite parameter space $ \mathcal{A}_n $ using the $ K $-fold cross-validation scheme. Let $ \esl_{ \tilde{a}_n } $ be the oracle ensemble learner obtained by applying the oracle selector over the same set of parameters. If the conditions of \Cref{finitesample} are satisfied, then the following oracle inequality holds
    \begin{align*}
...
\end{theorem}
\begin{proof}
    Replace $ E_{\Sn} $ in \Cref{finitesample} with the average over the $ K $ folds and $ n_1$ with $n/K $, then by using $ p = 1 $ the desired result is obtained.  
\end{proof}
...
\end{corollary}
\begin{proof}
    The proof is analogous to \Cref{cor:dslasymptoticequivalence}. We will replace $ \delta $ with $ \delta_n = 1/ \log(n) $. Note that in the remainder, the $ \log(1 + k(n)) $ term is less than $ 1 + \log(k(n)) = 1 + p \log(n) $, which is of lower order than $ n $. The $(1 + \delta_n)/ \delta_n $ term simplifies to become $ 1 + \log(n) $, and when multiplied with the $ \log(1 + k(n)) $ term, becomes of order $ O(\log(n)^2) $ that is still of lower order than $ n $. Thus the remainder term goes to zero as $ n \to \infty $ and the asymptotic result follows. 
\end{proof}
...
\end{align*}
This is clearly possible by \Cref{fig:simplex}. It thus follows that we can find a sequence of points $ b_n \in \mathcal{A}_n $ such that $ \meta_{b_n} \to \meta_{a} $ for any $ a \in \mathcal{A}$. The conclusion is that if $ \tilde{a} $ specified the learner $ \meta_{ \tilde{a} } $ with the minimum risk, then by applying the cross-validation selector and letting $ n \to \infty $, the meta learner adopted by the ensemble super learner will eventually converge to $ \meta_{ \tilde{a} } $. 
...
\begin{figure}[H]
    ...
    \caption{Raw predictions vs stratified according to the learner with highest weight as seen in \Cref{fig:esl_preds_lw_stratified}. The predictions of the locally weighted ensemble super is on the left, the right plot shows which learner was weighted the most in the $ k = 4 $ clusters.}
...
\begin{figure}[H]
    ...
    \caption{Another example similar to \Cref{fig:esl_preds_lw_stratified_tiled2}}
...
\begin{figure}[H]
    ...
    \caption{An example where XGBoost is weighted the highest when the predicted probabilities are around $ p = 0.5 $ and above $ 0.7$.}

...
\end{definition}
Here the $ P_n $ indicates that we fit each learner in the library on the entire data set, then a meta learner, $ \meta $, is used to combine the predictions made by each learner. 
...
The meta learning algorithm applied to $ \lone_{n} $ returns $ \meta(\cdot \mid \hat{a}) $, which is the meta learner with the empirical lowest risk on the level 1 data. We will now let the finite parameter set to depend on $ n $, such that the number elements in $ \mathcal{A}_{n} $ grows at most at a polynomial rate in $ n $. It is possible to write 
\begin{align*}
...
\begin{theorem}[Finite sample bound]
    \begin{align*}
        \frac{1}{K} \sum_{s = 1}^{K} E_{D_n} R(\esl_{ \hat{a}_n }(P_n) , P) \leq (1 + 2 \delta) E_{D_n} R(\esl_{ \tilde{a}_n }(P_n), P) + (1 + \delta) \frac{C \log(1 + K(n))}{\sqrt{n}}
    \end{align*}

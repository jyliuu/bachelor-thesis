...
\begin{corollary}[Asymptotic equivalence] 
   ...
   \begin{align*}
       E_{D_n} E_S R(\btheta_{ \tilde{\q} }(P_{n, S}^{0}), P) > \varepsilon \qquad \text{for all } n \in \mathbb{N},
   \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        \frac{E_{D_n} E_S R(\btheta_{\hat{\q}}(P_{n, S}^{0}), P)}{E_{D_n} E_{S} R(\btheta_{ \tilde{\q}}(P_{n,S}^{0}), P)} &\leq 1 + 2 \delta + \frac{(1 + \delta)E_S \frac{16}{\sqrt{n_1}} \log (1 +k) \sup_{\btheta \in \Theta} \left[ \frac{M(\theta)}{\sqrt{n_1}} + \sqrt{v(\theta)} \right]}{E_{D_n} E_{S} R(\btheta_{ \tilde{\q}}(P_{n,S}^{0}), P)} .
    \end{align*}
...
\end{align*}
for $\theta_0(x_1, x_2) = \expit({-3.5 - 0.3x_1 + 0.85x_2 + 0.35x_1x_2})$ which is the data-generatig regression function. The data-generating regression function is captured by the logistic regression model with interaction terms. We would like to learn the regression using the discrete super learner. We use the following library of learners as an illustrative example:
\begin{enumerate}
...
\end{enumerate}
Our goal is to estimate $\theta_0$ by applying the library above. By using the discrete super learner, we show that
\begin{enumerate}
    \item As the sample size increases, the  discrete super learner achieves the minimum risk in comparison to every other learner in the library 
    \item For a single new observation, the prediction of the discrete super learner on the outcome has the lowest variance among the learners in the library
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/dsl_loss_jumps.png}
    \caption{The loss of the discrete super learner compared to other learners. $ N = 3600 $}
\end{figure}

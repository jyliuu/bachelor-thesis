...
\end{figure}
Figure \ref{fig:learner_vars_w_esl_1000} compares the variances of the ensemble super learner to the other learners. The ensemble super learner does not exihibit the same behavior as the discrete super learner, which tends to predict many outliers; its distribution seems to be consistently centered, as evidenced in figure \ref{fig:hist_esl_dist}. This is coherent with the fact that the ensemble super learner is a continuous combination of different learners. We also note that the ensemble super learner achieves a lower variance than both the XGBoost and the discrete super learner, but has a variance than the parameteric logistic regression models. 
\begin{figure}[H]
    \centering
...
\subsection{Locally weighted ensemble super learner}
...
By examining the level 1 data obtained during the process of fitting an ensemble super learner fit, it is possible to compare the predictions of each learner against eachother as in figure \ref{fig:esl_preds_xgboost_vs_main}
\begin{figure}[H]
    \centering
...
\end{figure}
The figures above plots the predictions of two learners against eachother, but the procedure is valid also when there are more learners, resulting in clustering within a high dimensional space. For each group, a weighted combination of the learners would be found by fitting on the level 1 data corresponding to that group. To then predict using the ensemble super learner we will first apply the learners to the new observation, and determine which of the clusters the predicted probabilities belong to. Once that is determined, the predictions will be combined according to the weighted combination corresponding to that group. 

\subsubsection{Simulation results}
The locally weighted ensemble super learner was implemented using $ k = 4 $ clusters and the same library of learners as in the previous section. The results are shown in figure \ref{fig:esl_preds_lw}

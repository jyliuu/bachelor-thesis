...
\begin{frame}
    ...
    The conditional expectation $$ x \mapsto E(Y \mid X = x) = P(Y \mid X = x) $$ minimizes the  mean squared error (MSE) or risk over \(\Theta\)
        ...
        $$
        % is minimized. It turns out that  is what minimizes the MSE. We refer to it as the \textbf{regression}.  
      \end{frame}
% \begin{frame}
...
\begin{frame}
    ...
    We observe $ D_n = (O_1 , \dots , O_n) \in \mathcal{O}^{n}$. %upon which we apply our \textbf{learning algorithms}  
...
\begin{frame}
  ...
  % Instead of choosing the best learner, we can combine them.
  ...
  \vfill
    ...
    % \begin{definition}[Meta learner]
      % The \textit{meta learner} is a function $ \meta $ that maps the output of the candidate learners to a prediction:
      % $$\meta()$$
    % \end{definition}
